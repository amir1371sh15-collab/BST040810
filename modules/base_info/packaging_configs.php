...
// Define the families we want to filter by.
$allowed_families = ['بست بزرگ', 'بست کوچک'];
// Create SQL placeholders for the WHERE IN clause (e.g., "?,?")
$placeholders = implode(',', array_fill(0, count($allowed_families), '?'));

// Query for the main table, filtered by the allowed families.
$items_query = "
    SELECT 
        pc.PackageConfigID,
...
<div class="page-header d-flex justify-content-between align-items-center">
    <h1 class="h3 mb-0">پیکربندی بسته‌بندی (محصولات بست)</h1>
...
<div class="row">
    <div class="col-lg-4">
        <div class="card content-card"><div class="card-header"><h5 class="mb-0"><?php echo $editMode ? 'ویرایش پیکربندی' : 'افزودن پیکربندی جدید'; ?></h5></div><div class="card-body">
            <form method="POST" action="packaging_configs.php">
                ...
                <?php if ($editMode): ?><a href="packaging_configs.php" class="btn btn-secondary">لغو</a><?php endif; ?>
            </form>
        </div></div>
    </div>
    <div class="col-lg-8">
        <div class="card content-card"><div class="card-header"><h5 class="mb-0">لیست پیکربندی‌های بسته‌بندی</h5></div><div class="card-body p-0"><div class="table-responsive"><table class="table table-striped table-hover mb-0">
                ...
                <tr>
                    ...
                    <td class="p-3">
                        ...
                        if (!empty($item['UnitWeight'])) {
                            ...
                            echo number_format($totalWeight, 3) . ' ' . htmlspecialchars($item['UnitSymbol'] ?? 'kg');
                        } else {
                    ...
                    <td class="p-3">
                        ...
                        <div class="modal fade" id="deleteModal<?php echo $item[PRIMARY_KEY]; ?>" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
                            ...
                            <div class="modal-footer">
                                <form method="POST" action="packaging_configs.php"><input type="hidden" name="delete_id" value="<?php echo $item[PRIMARY_KEY]; ?>"><button type="submit" class="btn btn-danger">بله</button></form>

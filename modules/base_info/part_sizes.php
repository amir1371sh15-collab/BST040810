...
const TABLE_NAME = 'tbl_part_sizes';
const PRIMARY_KEY = 'SizeID';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['delete_id'])) {
        ...
        if (empty($data['FamilyID']) || empty($data['SizeName'])) {
             $result = ['success' => false, 'message' => 'خانواده و نام سایز الزامی است.'];
    ...
    header("Location: " . BASE_URL . "modules/base_info/part_sizes.php");
    exit;
}
...
$items = find_all($pdo, "SELECT ps.*, pf.FamilyName FROM " . TABLE_NAME . " ps JOIN tbl_part_families pf ON ps.FamilyID = pf.FamilyID ORDER BY pf.FamilyName, ps.SizeName");
$families = find_all($pdo, "SELECT * FROM tbl_part_families ORDER BY FamilyName");

$pageTitle = "مدیریت سایز قطعات";
include __DIR__ . '/../../templates/header.php';
?>
<div class="page-header d-flex justify-content-between align-items-center">
    <h1 class="h3 mb-0">مدیریت سایز قطعات</h1>
...
<div class="row">
    <div class="col-lg-4">
        <div class="card content-card"><div class="card-header"><h5 class="mb-0"><?php echo $editMode ? 'ویرایش سایز' : 'افزودن سایز جدید'; ?></h5></div><div class="card-body">
            <form method="POST" action="part_sizes.php">
                ...
                <div class="mb-3"><label class="form-label">نام سایز</label><input type="text" class="form-control" name="size_name" value="<?php echo htmlspecialchars($itemToEdit['SizeName'] ?? ''); ?>" required></div>
                <button type="submit" class="btn <?php echo $editMode ? 'btn-success' : 'btn-primary'; ?>"><?php echo $editMode ? 'بروزرسانی' : 'افزودن'; ?></button>
                <?php if ($editMode): ?><a href="part_sizes.php" class="btn btn-secondary">لغو</a><?php endif; ?>
            </form>
        </div></div>
    </div>
    <div class="col-lg-8">
        <div class="card content-card"><div class="card-header"><h5 class="mb-0">لیست سایزها</h5></div><div class="card-body p-0"><div class="table-responsive"><table class="table table-striped table-hover mb-0">
            <thead><tr><th class="p-3">خانواده</th><th class="p-3">سایز</th><th class="p-3">عملیات</th></tr></thead>
            <tbody><?php foreach ($items as $item): ?><tr>
                ...
                <td class="p-3">
                    ...
                    <div class="modal fade" id="deleteModal<?php echo $item[PRIMARY_KEY]; ?>" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
                        ...
                        <div class="modal-body">آیا از حذف سایز "<?php echo htmlspecialchars($item['SizeName']); ?>" برای خانواده "<?php echo htmlspecialchars($item['FamilyName']); ?>" مطمئن هستید؟</div>
                        <div class="modal-footer">
                            <form method="POST" action="part_sizes.php"><input type="hidden" name="delete_id" value="<?php echo $item[PRIMARY_KEY]; ?>"><button type="submit" class="btn btn-danger">بله</button></form>

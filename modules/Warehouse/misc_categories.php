...
const TABLE_NAME = 'tbl_misc_categories';
const PRIMARY_KEY = 'CategoryID';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['delete_id'])) {
        ...
        $data = ['CategoryName' => trim($_POST['category_name'])];
        if (empty($data['CategoryName'])) {
             $result = ['success' => false, 'message' => 'نام دسته‌بندی نمی‌تواند خالی باشد.'];
    ...
    header("Location: " . BASE_URL . "modules/Warehouse/misc_categories.php");
    exit;
}
...
$items = find_all($pdo, "SELECT * FROM " . TABLE_NAME . " ORDER BY CategoryName");

$pageTitle = "مدیریت دسته‌بندی مواد متفرقه";
...
<div class="row">
    <div class="col-lg-4">
        <div class="card content-card"><div class="card-header"><h5 class="mb-0"><?php echo $editMode ? 'ویرایش دسته‌بندی' : 'افزودن دسته‌بندی جدید'; ?></h5></div><div class="card-body">
            <form method="POST" action="misc_categories.php">
                <?php if ($editMode && $itemToEdit): ?><input type="hidden" name="id" value="<?php echo $itemToEdit[PRIMARY_KEY]; ?>"><?php endif; ?>
                <div class="mb-3"><label class="form-label">نام دسته‌بندی</label><input type="text" class="form-control" name="category_name" value="<?php echo htmlspecialchars($itemToEdit['CategoryName'] ?? ''); ?>" required></div>
                <button type="submit" class="btn <?php echo $editMode ? 'btn-success' : 'btn-primary'; ?>"><?php echo $editMode ? 'بروزرسانی' : 'افزودن'; ?></button>
                <?php if ($editMode): ?><a href="misc_categories.php" class="btn btn-secondary">لغو</a><?php endif; ?>
            </form>
        </div></div>
    </div>
    <div class="col-lg-8">
        <div class="card content-card"><div class="card-header"><h5 class="mb-0">لیست دسته‌بندی‌ها</h5></div><div class="card-body p-0"><div class="table-responsive"><table class="table table-striped table-hover mb-0">
            <thead><tr><th class="p-3">نام دسته‌بندی</th><th class="p-3">عملیات</th></tr></thead>
            <tbody><?php foreach ($items as $item): ?><tr>
                <td class="p-3"><?php echo htmlspecialchars($item['CategoryName']); ?></td>
                <td class="p-3">
                    ...
                    <div class="modal fade" id="deleteModal<?php echo $item[PRIMARY_KEY]; ?>" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
                        ...
                        <div class="modal-body">آیا از حذف دسته‌بندی "<?php echo htmlspecialchars($item['CategoryName']); ?>" مطمئن هستید؟</div>
                        <div class="modal-footer">
                            <form method="POST" action="misc_categories.php"><input type="hidden" name="delete_id" value="<?php echo $item[PRIMARY_KEY]; ?>"><button type="submit" class="btn btn-danger">بله</button></form>

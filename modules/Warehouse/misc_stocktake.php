...
if (!has_permission('warehouse.misc.manage')) { die('شما مجوز انجام عملیات انبارگردانی را ندارید.'); }

const TABLE_NAME = 'tbl_misc_transactions';
const PRIMARY_KEY = 'TransactionID';
const RECORDS_PER_PAGE = 15;
// استفاده از همان انواع تراکنش انبارگردانی
...
$quotedTypeNames = implode(',', array_map(function ($name) use ($pdo) { return $pdo->quote($name); }, STOCKTAKE_TYPE_NAMES));
...
$initial_log_date_jalali = $_SESSION['misc_stocktake_log_date'] ?? to_jalali(date('Y-m-d'));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_id'])) {
    // ... (کد حذف مشابه inventory_stocktake.php) ...
    ...
    if (!$delete_id) {
        ...
        if (!$transaction || !in_array((int)$transaction['TransactionTypeID'], $stocktake_type_ids, true)) {
            $_SESSION['message'] = 'رکورد انبارگردانی یافت نشد.';
            $_SESSION['message_type'] = 'warning';
        } else {
    ...
    // ... (کد محاسبه صفحه ریدایرکت) ...
    header("Location: " . BASE_URL . "modules/Warehouse/misc_stocktake.php?page=" . $redirect_page);
    exit;
}
...
if (isset($_GET['edit_id'])) {
    ...
    if ($edit_id) {
        ...
        if ($transaction && in_array((int)$transaction['TransactionTypeID'], $stocktake_type_ids, true)) {
            ...
            $itemInfo = find_by_id($pdo, 'tbl_misc_items', $transaction['ItemID'], 'ItemID');
            if (!$itemInfo) {
                $_SESSION['message'] = 'اطلاعات ماده متفرقه برای ویرایش یافت نشد.';
                $_SESSION['message_type'] = 'warning';
                header("Location: " . BASE_URL . "modules/Warehouse/misc_stocktake.php?page=" . $current_page);
                exit;
            }
            ...
            $_SESSION['misc_stocktake_log_date'] = $itemToEdit['TransactionDateJalali'];
...
$pageTitle = "انبارگردانی مواد متفرقه";
...
<form id="stocktake-form">
    <div class="card content-card mb-4">
        <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <?php if ($editMode): ?>
                <a href="misc_stocktake.php?page=<?php echo $current_page; ?>" class="btn btn-sm btn-outline-secondary">
        ...
        <div class="card-body">
            ...
            <div class="row">
                <div class="col-md-3 mb-3">
                    ...
                    <input type="text" id="transaction_date" name="transaction_date" class="form-control persian-date persistent-input" data-session-key="misc_stocktake_log_date"
            ...
            <div class="row">
                 ...
                 <div class="col-md-4 mb-3">
                    ...
                    <input type="text" id="description" name="description" class="form-control" placeholder="دلیل انبارگردانی...">
                </div>
            </div>

            <!-- Container for Current Balances -->
            <div id="current-balances-container" class="mt-3 mb-3" style="display: none;">
                <h6>موجودی فعلی (تا تاریخ <span id="balance-date-display"></span>) برای <span id="balance-item-display"></span>:</h6>
                <div class="table-responsive">
                    <table class="table table-sm table-bordered" id="current-balances-table">
                         <thead><tr class="table-secondary small"><th class="text-center" id="balance-unit-header">موجودی (?)</th></tr></thead>
            ...
            <div class="row align-items-end">
                 <div class="col-md-4 mb-3">
                     ...
                     <div class="input-group">
                        <input type="number" step="0.01" id="quantity" name="quantity" class="form-control" placeholder="مقدار اولیه یا مقدار تعدیل" required>
...
<div class="modal fade" id="deleteConfirmModal" tabindex="-1">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">آیا از حذف این رکورد انبارگردانی مطمئن هستید؟</div>
      <div class="modal-footer">
          <form id="delete-form" method="POST" action="misc_stocktake.php?page=<?php echo $current_page; ?>" class="d-inline">
...
$(document).ready(function() {
    const apiRecordUrl = '<?php echo BASE_URL; ?>api/api_misc_record_transaction.php';
    const apiGetItemsUrl = '<?php echo BASE_URL; ?>api/api_misc_get_items.php';
    const apiGetInventoryUrl = '<?php echo BASE_URL; ?>api/api_misc_get_current_inventory.php';
    ...
    const balanceUnitHeader = $('#balance-unit-header');
    const deleteTransactionIdInput = $('#delete-transaction-id');
    
    const stocktakePageUrl = 'misc_stocktake.php';
    ...
    transactionDateInput.on('change', function() {
        ...
        $.post('<?php echo BASE_URL; ?>api/api_update_session.php', { key: 'misc_stocktake_log_date', value: newDate });
        ...
        try {
            ...
            if (recordResponse.success) {
                ...
                if (editMode) {
                    ...
                    redirectUrl += `?focus_transaction_type=true`; // Focus type field
                }
        ...
        try {
            ...
            if (response.success && response.data.length > 0) {
                response.data.forEach(item => {
                    itemDataCache[item.ItemID] = item.UnitSymbol || '??'; // Store unit
                    itemSelect.append($('<option>', { value: item.ItemID, text: item.ItemName }));
                });
                itemSelect.prop('disabled', false);
                if (selectedItemId) {
                    itemSelect.val(selectedItemId);
                    updateQuantityUnit(); // Update unit based on selected item
                }
            } else {
    ...
    function checkAndLoadBalances() {
        ...
        if ((typeName.includes('کسر') || typeName.includes('اضافه')) && itemId && dateJalali) {
            loadCurrentBalances(itemId, dateJalali);
        } else {
            balanceContainer.hide();
        }
    }

    async function loadCurrentBalances(itemId, dateJalali) {
        balanceContainer.show();
        balanceTableBody.html('<tr><td class="text-center text-muted"><div class="spinner-border spinner-border-sm"></div></td></tr>');
        const itemName = itemSelect.find('option:selected').text();
        const unit = itemDataCache[itemId] || '??';

        $('#balance-date-display').text(dateJalali);
        $('#balance-item-display').text(itemName);
        balanceUnitHeader.text(`موجودی (${unit})`);
        ...
        try {
            const response = await $.getJSON(apiGetInventoryUrl, {
             ...
             if (response.success) {
                 const balanceValue = parseFloat(response.data.CurrentBalance).toFixed(3);
                 balanceTableBody.append(`<tr><td class="text-center small">${balanceValue}</td></tr>`);
             } else {
                  balanceTableBody.html(`<tr><td class="text-center text-warning small">${response.message || 'خطا در واکشی موجودی.'}</td></tr>`);
             }
        } catch (error) {
            console.error("Error fetching current misc balance:", error);
            balanceTableBody.html('<tr><td class="text-center text-danger small">خطای شبکه در دریافت موجودی.</td></tr>');
        }
    }
    ...
    // --- Initial Load ---
    initializeEditMode(); // Populate edit fields if applicable

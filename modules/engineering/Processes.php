...
const TABLE_NAME = 'tbl_processes';
const PRIMARY_KEY = 'ProcessID';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['delete_id'])) {
        ...
        $data = ['ProcessName' => trim($_POST['process_name'])];
        if (empty($data['ProcessName'])) {
            $result = ['success' => false, 'message' => 'نام فرآیند نمی‌تواند خالی باشد.'];
    ...
    header("Location: " . BASE_URL . "modules/engineering/processes.php");
    exit;
}
...
$items = find_all($pdo, "SELECT * FROM " . TABLE_NAME . " ORDER BY ProcessName");

$pageTitle = "مدیریت فرآیندهای مهندسی";
include __DIR__ . '/../../templates/header.php';
?>
<div class="page-header d-flex justify-content-between align-items-center">
    <h1 class="h3 mb-0">مدیریت فرآیندهای مهندسی</h1>
...
<div class="row">
    <div class="col-lg-4">
        <div class="card content-card">
            <div class="card-header"><h5 class="mb-0"><?php echo $editMode ? 'ویرایش فرآیند' : 'افزودن فرآیند جدید'; ?></h5></div>
            <div class="card-body">
                <form method="POST" action="processes.php">
                    ...
                    <div class="mb-3">
                        <label for="process_name" class="form-label">نام فرآیند</label>
                        <input type="text" class="form-control" id="process_name" name="process_name" value="<?php echo htmlspecialchars($itemToEdit['ProcessName'] ?? ''); ?>" required>
                    </div>
                    <button type="submit" class="btn <?php echo $editMode ? 'btn-success' : 'btn-primary'; ?>"><?php echo $editMode ? 'بروزرسانی' : 'افزودن'; ?></button>
                    <?php if ($editMode): ?><a href="processes.php" class="btn btn-secondary">لغو</a><?php endif; ?>
                </form>
            </div>
        </div>
    </div>
    <div class="col-lg-8">
        <div class="card content-card">
            <div class="card-header"><h5 class="mb-0">لیست فرآیندها</h5></div>
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-striped table-hover mb-0">
                        <thead><tr><th class="p-3">نام فرآیند</th><th class="p-3">عملیات</th></tr></thead>
                        <tbody>
                            <?php foreach ($items as $item): ?>
                            <tr>
                                <td class="p-3"><?php echo htmlspecialchars($item['ProcessName']); ?></td>
                                <td class="p-3">
                                    ...
                                    <div class="modal fade" id="deleteModal<?php echo $item[PRIMARY_KEY]; ?>" tabindex="-1">
                                      <div class="modal-dialog"><div class="modal-content">
                                        ...
                                        <div class="modal-body">آیا از حذف فرآیند "<?php echo htmlspecialchars($item['ProcessName']); ?>" مطمئن هستید؟</div>
                                        <div class="modal-footer">
                                          <form method="POST" action="processes.php"><input type="hidden" name="delete_id" value="<?php echo $item[PRIMARY_KEY]; ?>"><button type="submit" class="btn btn-danger">بله، حذف کن</button></form>

...
const TABLE_NAME = 'tbl_maintenance_causes';
const PRIMARY_KEY = 'CauseID';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['delete_id'])) {
        ...
        if (empty(trim($_POST['cause_description']))) {
            $result = ['success' => false, 'message' => 'شرح دلیل نمی‌تواند خالی باشد.'];
            $_SESSION['message_type'] = 'warning';
        } else {
            $data = ['CauseDescription' => trim($_POST['cause_description'])];
    ...
    header("Location: " . BASE_URL . "modules/engineering/maintenance_causes.php");
    exit;
}
...
$items = find_all($pdo, "SELECT * FROM " . TABLE_NAME . " ORDER BY CauseDescription");

$pageTitle = "مدیریت دلایل نت";
include __DIR__ . '/../../templates/header.php';
?>

<div class="page-header d-flex justify-content-between align-items-center">
    <h1 class="h3 mb-0">مدیریت دلایل نت</h1>
...
<div class="row">
    <div class="col-lg-4">
        <div class="card content-card">
            ...
            <div class="card-body">
                <form method="POST" action="maintenance_causes.php">
                    <?php if ($editMode && $itemToEdit): ?>
                        <input type="hidden" name="id" value="<?php echo $itemToEdit[PRIMARY_KEY]; ?>">
                    <?php endif; ?>
                    <div class="mb-3">
                        <label for="cause_description" class="form-label">شرح دلیل</label>
                        <input type="text" class="form-control" id="cause_description" name="cause_description" value="<?php echo htmlspecialchars($itemToEdit['CauseDescription'] ?? ''); ?>" required>
                    </div>
                    <button type="submit" class="btn <?php echo $editMode ? 'btn-success' : 'btn-primary'; ?>"><?php echo $editMode ? 'بروزرسانی' : 'افزودن'; ?></button>
                    <?php if ($editMode): ?><a href="maintenance_causes.php" class="btn btn-secondary">لغو</a><?php endif; ?>
                </form>
            </div>
        </div>
    </div>
                ...
                <div class="table-responsive">
                    <table class="table table-striped table-hover mb-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td class="p-3"><?php echo htmlspecialchars($item['CauseDescription']); ?></td>
                                <td class="p-3">
                                    ...
                                    <div class="modal fade" id="deleteModal<?php echo $item[PRIMARY_KEY]; ?>" tabindex="-1">
                                      <div class="modal-dialog"><div class="modal-content">
                                        ...
                                        <div class="modal-body">آیا از حذف دلیل "<?php echo htmlspecialchars($item['CauseDescription']); ?>" مطمئن هستید؟</div>
                                        <div class="modal-footer">
                                          <form method="POST" action="maintenance_causes.php" class="d-inline"><input type="hidden" name="delete_id" value="<?php echo $item[PRIMARY_KEY]; ?>"><button type="submit" class="btn btn-danger">بله، حذف کن</button></form>

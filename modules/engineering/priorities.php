...
const TABLE_NAME = 'tbl_priorities';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['delete_id'])) {
        ...
        if (empty(trim($_POST['priority_name']))) {
            $result = ['success' => false, 'message' => 'نام اولویت نمی‌تواند خالی باشد.'];
            $_SESSION['message_type'] = 'warning';
        } else {
            $data = ['PriorityName' => trim($_POST['priority_name'])];
    ...
    header("Location: " . BASE_URL . "modules/engineering/priorities.php");
    exit;
}
...
$items = find_all($pdo, "SELECT * FROM " . TABLE_NAME . " ORDER BY PriorityID");

$pageTitle = "مدیریت اولویت‌ها";
include __DIR__ . '/../../templates/header.php';
?>

<div class="page-header d-flex justify-content-between align-items-center">
    <h1 class="h3 mb-0">مدیریت اولویت‌ها</h1>
...
<div class="row">
    <div class="col-lg-4">
        <div class="card content-card">
            <div class="card-header"><h5 class="mb-0"><?php echo $editMode ? 'ویرایش اولویت' : 'افزودن اولویت جدید'; ?></h5></div>
            <div class="card-body">
                <form method="POST" action="priorities.php">
                    <?php if ($editMode && $itemToEdit): ?>
                        <input type="hidden" name="id" value="<?php echo $itemToEdit[PRIMARY_KEY]; ?>">
                    <?php endif; ?>
                    <div class="mb-3">
                        <label for="priority_name" class="form-label">نام اولویت</label>
                        <input type="text" class="form-control" id="priority_name" name="priority_name" value="<?php echo htmlspecialchars($itemToEdit['PriorityName'] ?? ''); ?>" required>
                    </div>
                    <button type="submit" class="btn <?php echo $editMode ? 'btn-success' : 'btn-primary'; ?>"><?php echo $editMode ? 'بروزرسانی' : 'افزودن'; ?></button>
                    <?php if ($editMode): ?><a href="priorities.php" class="btn btn-secondary">لغو</a><?php endif; ?>
                </form>
            </div>
        </div>
    </div>
    <div class="col-lg-8">
        <div class="card content-card">
            <div class="card-header"><h5 class="mb-0">لیست اولویت‌ها</h5></div>
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-striped table-hover mb-0">
                        <thead><tr><th class="p-3">نام اولویت</th><th class="p-3">عملیات</th></tr></thead>
                        <tbody>
                                ...
                                <td class="p-3">
                                    ...
                                    <div class="modal fade" id="deleteModal<?php echo $item[PRIMARY_KEY]; ?>" tabindex="-1">
                                      <div class="modal-dialog"><div class="modal-content">
                                        ...
                                        <div class="modal-body">آیا از حذف اولویت "<?php echo htmlspecialchars($item['PriorityName']); ?>" مطمئن هستید؟</div>
                                        <div class="modal-footer">
                                          <form method="POST" action="priorities.php" class="d-inline"><input type="hidden" name="delete_id" value="<?php echo $item[PRIMARY_KEY]; ?>"><button type="submit" class="btn btn-danger">بله، حذف کن</button></form>

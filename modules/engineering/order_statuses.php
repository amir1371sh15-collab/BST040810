...
const TABLE_NAME = 'tbl_order_statuses';
const PRIMARY_KEY = 'OrderStatusID';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['delete_id'])) {
        ...
        if (empty(trim($_POST['status_name']))) {
            $result = ['success' => false, 'message' => 'نام وضعیت نمی‌تواند خالی باشد.'];
            $_SESSION['message_type'] = 'warning';
        } else {
            $data = ['StatusName' => trim($_POST['status_name'])];
    ...
    header("Location: " . BASE_URL . "modules/engineering/order_statuses.php");
    exit;
}
...
$items = find_all($pdo, "SELECT * FROM " . TABLE_NAME . " ORDER BY StatusName");

$pageTitle = "مدیریت وضعیت‌های سفارش";
include __DIR__ . '/../../templates/header.php';
?>

<div class="page-header d-flex justify-content-between align-items-center">
    <h1 class="h3 mb-0">مدیریت وضعیت‌های سفارش</h1>
...
<div class="row">
    <div class="col-lg-4">
        <div class="card content-card">
            <div class="card-header"><h5 class="mb-0"><?php echo $editMode ? 'ویرایش وضعیت' : 'افزودن وضعیت جدید'; ?></h5></div>
            <div class="card-body">
                <form method="POST" action="order_statuses.php">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="status_name" name="status_name" value="<?php echo htmlspecialchars($itemToEdit['StatusName'] ?? ''); ?>" required>
                    </div>
                    <button type="submit" class="btn <?php echo $editMode ? 'btn-success' : 'btn-primary'; ?>"><?php echo $editMode ? 'بروزرسانی' : 'افزودن'; ?></button>
                    <?php if ($editMode): ?><a href="order_statuses.php" class="btn btn-secondary">لغو</a><?php endif; ?>
                </form>
            </div>
        </div>
    </div>
    <div class="col-lg-8">
        <div class="card content-card">
            <div class="card-header"><h5 class="mb-0">لیست وضعیت‌ها</h5></div>
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-striped table-hover mb-0">
                        <thead><tr><th class="p-3">نام وضعیت</th><th class="p-3">عملیات</th></tr></thead>
                        <tbody>
                                ...
                                <td class="p-3">
                                    ...
                                    <div class="modal fade" id="deleteModal<?php echo $item[PRIMARY_KEY]; ?>" tabindex="-1">
                                      <div class="modal-dialog"><div class="modal-content">
                                        ...
                                        <div class="modal-body">آیا از حذف وضعیت "<?php echo htmlspecialchars($item['StatusName']); ?>" مطمئن هستید؟</div>
                                        <div class="modal-footer">
                                          <form method="POST" action="order_statuses.php" class="d-inline"><input type="hidden" name="delete_id" value="<?php echo $item[PRIMARY_KEY]; ?>"><button type="submit" class="btn btn-danger">بله، حذف کن</button></form>

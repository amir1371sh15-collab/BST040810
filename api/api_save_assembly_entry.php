...
$daily_plan = trim($_POST['daily_plan'] ?? '');
$description = trim($_POST['description'] ?? ''); // Get description
$machine_id = !empty($_POST['machine_id']) ? (int)$_POST['machine_id'] : null;
$operator1_id = !empty($_POST['operator1_id']) ? (int)$_POST['operator1_id'] : null;
$part_id = !empty($_POST['part_id']) ? (int)$_POST['part_id'] : null;
$operator2_id = !empty($_POST['operator2_id']) ? (int)$_POST['operator2_id'] : null;
$production_grams_str = $_POST['production_grams'] ?? '';
...
if (!$log_date_str || !$available_time || !$machine_id || !$operator1_id || !$part_id || $production_grams_str === '') {
    http_response_code(400);
    $response['message'] = 'خطا: تاریخ، زمان در دسترس، اپراتور مونتاژ، دستگاه، محصول و میزان تولید الزامی است.';
    echo json_encode($response, JSON_UNESCAPED_UNICODE);
    exit;
}

if (!is_numeric($production_grams_str) || (float)$production_grams_str < 0) {
    http_response_code(400);
    $response['message'] = 'خطا: میزان تولید باید عددی غیرمنفی باشد.';
    echo json_encode($response, JSON_UNESCAPED_UNICODE);
    exit;
}
$production_grams = (float)$production_grams_str;
$production_kg = $production_grams / 1000.0;
...
try {
    ...
    $header_data_for_update = [
        ...
        'DailyProductionPlan' => $daily_plan,
        'Description' => $description // Include description
    ];

    if ($entry_id_to_update) {
        $stmt = $pdo->prepare("SELECT AssemblyHeaderID FROM tbl_assembly_log_entries WHERE AssemblyEntryID = ?");
        ...
        update_record($pdo, 'tbl_assembly_log_header', $header_data_for_update, $header_id, 'AssemblyHeaderID');
    } else {
        ...
        if ($header_id) {
            ...
            $header_res = insert_record($pdo, 'tbl_assembly_log_header', $header_data_for_insert);
            if (!$header_res['success']) throw new Exception("خطا در ایجاد هدر گزارش.");
    ...
    if ($entry_id_to_update) {
        $stmt = $pdo->prepare("UPDATE tbl_assembly_log_entries SET MachineID=?, Operator1ID=?, Operator2ID=?, StartTime=?, EndTime=?, PartID=?, ProductionKG=? WHERE AssemblyEntryID=?");
        $success = $stmt->execute([$machine_id, $operator1_id, $operator2_id, $start_time, $end_time, $part_id, $production_kg, $entry_id_to_update]);
        $message = $success ? 'رکورد با موفقیت ویرایش شد.' : 'خطا در ویرایش رکورد.';
    } else {
        $stmt = $pdo->prepare("INSERT INTO tbl_assembly_log_entries (AssemblyHeaderID, MachineID, Operator1ID, Operator2ID, StartTime, EndTime, PartID, ProductionKG) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $success = $stmt->execute([$header_id, $machine_id, $operator1_id, $operator2_id, $start_time, $end_time, $part_id, $production_kg]);
    ...
    $pdo->rollBack();
    error_log("API Save Assembly Error: " . $e->getMessage());

...
try {
    ...
    // *** NEW: Fetch active break times (relevant for assembly, assuming NULL department applies) ***
    $assembly_dept_id = 3; // Assuming assembly department ID is 3, adjust if needed
    ...
    // Convert break times to seconds from midnight for easier comparison
    $break_times_seconds = [];
    foreach ($break_times_raw as $break) {
        ...
        if ($start_secs !== false && $end_secs !== false && $end_secs > $start_secs) {
            $break_times_seconds[] = ['start' => $start_secs, 'end' => $end_secs, 'duration' => $end_secs - $start_secs];
        }
    }
    // *** END NEW ***
    ...
    $sql = "
        SELECT
            ...
            -- Calculate raw active seconds per day first
            COALESCE(SUM(TIMESTAMPDIFF(SECOND, e.StartTime, e.EndTime)), 0) as RawActiveSeconds
        ...
        {$where_sql_header}
        AND e.StartTime IS NOT NULL AND e.EndTime IS NOT NULL AND e.EndTime >= e.StartTime -- Only consider valid time entries
        GROUP BY h.LogDate, h.AvailableTimeMinutes, h.DailyProductionPlan
        ORDER BY h.LogDate ASC
    ";

    $daily_data_raw = find_all($pdo, $sql, $params_header);
    $daily_data = array_column($daily_data_raw, null, 'LogDate'); // Index by date
    ...
    foreach ($all_entries as $entry) {
        ...
        // Ensure we don't subtract more break time than the entry duration
    ...
    while ($current_date <= $end_date_dt) {
        ...
        $dayOfWeek = $current_date->format('w'); // 0 (for Sunday) through 6 (for Saturday)

        if (isset($daily_data[$date_str])) {
            $day_info = $daily_data[$date_str];
            // Convert adjusted seconds to hours
            $adjusted_hours = isset($adjusted_active_seconds_per_day[$date_str]) ? round($adjusted_active_seconds_per_day[$date_str] / 3600, 2) : 0;

            $result_data[$date_str] = [
                'LogDate' => $date_str,
                'LogDateJalali' => to_jalali($date_str),
                'AvailableTimeMinutes' => (int)($day_info['AvailableTimeMinutes'] ?? 0),
                'DailyProductionPlan' => (int)($day_info['DailyProductionPlan'] ?? 0), // Assuming plan is count
                'ActiveMachines' => (int)$day_info['ActiveMachines'],
                'ActualCount' => round((float)$day_info['ActualCount']),
                'ActualActiveHours' => $adjusted_hours, // Use adjusted hours
                'dayOfWeek' => (int)$dayOfWeek
            ];
        } else {
             // Try to fetch header info even for days with no entries
    ...
    // Return as indexed array for easier iteration in JS
    $response['data'] = array_values($result_data);

} catch (Exception $e) {
    error_log("Assembly Log Report API Error: " . $e->getMessage());

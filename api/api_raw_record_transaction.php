...
if (!has_permission('warehouse.raw.manage')) {
...
try {
    ...
    if ($transaction_id) {
        $existing_tx = find_by_id($pdo, 'tbl_raw_transactions', $transaction_id, 'TransactionID');
    ...
    if ($transaction_id) { // Update
        ...
        $result = update_record($pdo, 'tbl_raw_transactions', $data, $transaction_id, 'TransactionID');
        $message = $result['success'] ? 'تراکنش با موفقیت ویرایش شد.' : 'خطا در ویرایش تراکنش.';
    } else { // Insert
        $result = insert_record($pdo, 'tbl_raw_transactions', $data);
    ...
    $pdo->rollBack();
    error_log("API Record Raw Transaction Error: " . $e->getMessage() . " | POST Data: " . print_r($_POST, true));
